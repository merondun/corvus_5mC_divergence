---
title: "Crow hybrid zone epigenetics"
...
set.seed(123)
# Read file
d2 = read.table('5mC_10x_STAT-Region_20250107.txt',header=TRUE)
d3 <- d2 %>% 
...
sid=NULL
for (si in unique(hz$site)) {
  s1 = hz %>% filter(site == si) %>% select(matches('site|.hz')) %>% 
    select(matches('site|^S_|^H_|^D_')) %>% 
    pivot_longer(!(site),names_to='id',values_to = 'mC') %>% dplyr::select(-site) %>% na.omit()
  s1 <- s1 %>% mutate(id = gsub('.hz','',id))
...
pdf('20250107_Clustered_Genotypes.pdf',height=2.5,width=8)
hip
dev.off()
